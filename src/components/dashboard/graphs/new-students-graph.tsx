...
export function NewStudentsGraph({ organizationId }: { organizationId: string | null }) {
  ...
  const processChartData = useCallback((records: Student[], range: DateRange) => {
    ...
    const newStudentsByDay: { [key: string]: number } = {};
    newStudentsInRange.forEach(student => {
      const dateStr = format(student.joinDate.toDate(), 'yyyy-MM-dd');
      if (!newStudentsByDay[dateStr]) {
        newStudentsByDay[dateStr] = 0;
      }
      newStudentsByDay[dateStr]++;
    });
    ...
    const chartData = intervalDays.map(day => {
      ...
      return {
        ...
        "New Students": newStudentsByDay[dateKey] || 0,
      };
    });
      ...
      <CardContent className="flex-grow pb-4 -ml-4">
        {loading ? (
          ...
          <div className="flex h-[300px] items-center justify-center text-muted-foreground flex-col gap-2">
            <p className="text-sm">No new admissions found for the selected period.</p>
          </div>
        )}
      </CardContent>
    </Card>
